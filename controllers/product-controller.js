...
exports.postEditProduct = (req, res) => {
  let { title, pimage, description, price, category, special, vegan } =
    req.body;
  ...
  Product.findOne(
    { slug: slug, category: category, _id: { $ne: id } },
    (err, product) => {
...
exports.postEditProductAddGallery = async (req, res) => {
  let id = req.params.id;
  console.log(id);
  ...
  if (images.length > 0) {
    ...
    await imageName.map(img => {
      Product.findByIdAndUpdate({ _id: id }, { $push: { images: img } }).then(
        pro => {
          pro.save(() => {
            console.log('saved');
          });
...
exports.getEditProductAddGallery = (req, res) => {
  let id = req.params.id;
